...
import { PlusCircle, Edit, Trash2, LayoutDashboard, Users, ClipboardList, Handshake, Search, Calendar, AlertCircle, TrendingUp, TrendingDown, ChevronLeft, ChevronRight, X, BookHeart, LogOut } from 'lucide-react';
...
const LoginScreen = ({ setToken, setUser }) => {
    ...
    const handleLogin = async (e) => {
        ...
        setError('');
        try {
            const response = await fetch('https://clinicflow-backend.onrender.com/api/auth/login', {
                method: 'POST',
            ...
            localStorage.setItem('clinicflow_user', JSON.stringify(data.usuario));
        } catch (err) {
    ...
    return (
        <div className="flex items-center justify-center h-screen bg-gray-100">
            <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-xl shadow-lg">
                <h1 className="text-3xl font-bold text-center text-indigo-600">ClinicFlow</h1>
                <form onSubmit={handleLogin} className="space-y-6">
                    <input type="email" placeholder="E-mail (ex: admin@clinicflow.com)" value={email} onChange={e => setEmail(e.target.value)} required className="w-full px-4 py-2 border rounded-lg" />
                    <input type="password" placeholder="Senha (ex: admin)" value={senha} onChange={e => setSenha(e.target.value)} required className="w-full px-4 py-2 border rounded-lg" />
                    {error && <p className="text-red-500 text-sm text-center">{error}</p>}
                    <button type="submit" disabled={isLoading} className="w-full px-4 py-2 font-semibold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 disabled:bg-indigo-400">
...
const Sidebar = ({ currentPage, setCurrentPage, user, onLogout }) => {
    const navItems = [
        ...
        { name: 'Agenda', icon: Calendar, roles: ['admin', 'fisioterapeuta'] },
        { name: 'Clientes', icon: Users, roles: ['admin', 'fisioterapeuta'] },
        { name: 'Serviços', icon: ClipboardList, roles: ['admin'] },
        { name: 'Profissionais', icon: Handshake, roles: ['admin'] },
    ];

    const accessibleNavItems = navItems.filter(item => item.roles.includes(user.perfil));
    ...
    return (
        <aside className="w-64 bg-white text-gray-800 flex flex-col shadow-lg">
            ...
            <nav className="flex-1 px-4 py-4">
                <ul>{accessibleNavItems.map(item => (<li key={item.name} className="mb-2"><a href="#" onClick={() => setCurrentPage(item.name)} className={`flex items-center p-3 rounded-lg transition-all duration-200 ${currentPage === item.name ? 'bg-indigo-600 text-white shadow-md' : 'hover:bg-gray-200'}`}><item.icon className="h-5 w-5 mr-3" /><span className="font-medium">{item.name}</span></a></li>))}</ul>
            </nav>
            <div className="p-4 border-t">
                ...
                <button onClick={onLogout} className="w-full flex items-center justify-center p-2 bg-red-500 text-white rounded-lg hover:bg-red-600"><LogOut className="h-5 w-5 mr-2" />Sair</button>
            </div>
        </aside>
    );
};
...
const ClientForm = ({ onBack, setClients, clientToEdit = null, token }) => {
...
const ProntuarioView = ({ client, token }) => {
    const [sessoes, setSessoes] = useState([]);
    const [isModalOpen, setIsModalOpen] = useState(false);
    
    useEffect(() => {
        if(client.id) {
            fetch(`https://clinicflow-backend.onrender.com/api/pacientes/${client.id}/prontuario`, { headers: { 'Authorization': `Bearer ${token}` }})
                .then(res => res.json())
            ...
            <div className="space-y-4">
                {(sessoes || []).map(sessao => (
                    <div key={sessao.id} className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div className="mt-2"><p className="font-semibold">Subjetivo:</p><p className="text-gray-700 whitespace-pre-wrap">{sessao.subjetivo}</p></div>
                         <div className="mt-2"><p className="font-semibold">Objetivo:</p><p className="text-gray-700 whitespace-pre-wrap">{sessao.objetivo}</p></div>
                         <div className="mt-2"><p className="font-semibold">Plano de Tratamento:</p><p className="text-gray-700 whitespace-pre-wrap">{sessao.plano_tratamento}</p></div>
                    </div>
                ))}
            </div>
            {isModalOpen && <SessaoFormModal client={client} closeModal={() => setIsModalOpen(false)} setSessoes={setSessoes} token={token} />}
        </div>
    );
};

const SessaoFormModal = ({ closeModal, client, setSessoes, token }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`https://clinicflow-backend.onrender.com/api/pacientes/${client.id}/prontuario`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify(payload) });
...
const ClientDetailView = ({ client, onBack, setClients, token }) => {
    ...
    return (
        <div>
            ...
            {activeTab === 'prontuario' ? <ProntuarioView client={client} token={token} /> : <ClientForm onBack={onBack} clientToEdit={client} setClients={setClients} token={token} />}
        </div>
    );
};

const Clients = ({ clients, setClients, token }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [selectedClient, setSelectedClient] = useState(null);

    if (selectedClient) {
        return <ClientDetailView client={selectedClient} onBack={() => setSelectedClient(null)} setClients={setClients} token={token} />;
    }
...
const Services = ({ services, setServices, token }) => {
    const [isModalOpen, setIsModalOpen] = useState(false);
    const handleDelete = async (serviceId) => {
        if (!window.confirm('Tem certeza que deseja excluir este serviço?')) return;
        try {
            const response = await fetch(`https://clinicflow-backend.onrender.com/api/servicos/${serviceId}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
    ...
    return (
        <div>
            ...
            <div className="bg-white rounded-xl shadow-md overflow-hidden">
                <table className="w-full text-left">
                    <thead className="bg-gray-50 border-b"><tr><th className="p-4 font-semibold text-gray-600">Nome</th><th className="p-4 font-semibold text-gray-600">Preço</th><th className="p-4 font-semibold text-gray-600">Duração (min)</th><th className="p-4 font-semibold text-gray-600">Ações</th></tr></thead>
            ...
            {isModalOpen && <NewServiceModal closeModal={() => setIsModalOpen(false)} setServices={setServices} token={token} />}
        </div>
    );
};

const NewServiceModal = ({ closeModal, setServices, token }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setIsSubmitting(true);
        try {
            const response = await fetch('https://clinicflow-backend.onrender.com/api/servicos', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ ...formData, preco: parseFloat(formData.preco), duracao_minutos: parseInt(formData.duracao_minutos) }) });
...
const Professionals = ({ professionals, setProfessionals, token }) => {
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingProfessional, setEditingProfessional] = useState(null);
    const handleDelete = async (id) => {
        if (!window.confirm('Tem certeza que deseja excluir este profissional?')) return;
        try {
            const response = await fetch(`https://clinicflow-backend.onrender.com/api/profissionais/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
    ...
    return (
        <div>
            ...
            {(isModalOpen || editingProfessional) && <ProfessionalFormModal professionalToEdit={editingProfessional} closeModal={() => { setIsModalOpen(false); setEditingProfessional(null); }} setProfessionals={setProfessionals} token={token} />}
        </div>
    );
};

const ProfessionalFormModal = ({ closeModal, setProfessionals, professionalToEdit = null, token }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const method = isEditing ? 'PUT' : 'POST';
        try {
            const response = await fetch(url, { method, headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ ...formData, comissao_percentual: parseInt(formData.comissao_percentual) }) });
            if (!response.ok) throw new Error((await response.json()).mensagem || 'Falha na operação.');
...
const Agenda = ({ appointments, setAppointments, clients, services, professionals, token }) => {
...
const AppointmentFormModal = ({ closeModal, setAppointments, clients, services, professionals, appointmentToEdit = null, selectedDate = null, token }) => {
    ...
    const handleDelete = async () => {
        ...
        setIsSubmitting(true);
        try {
            const response = await fetch(`https://clinicflow-backend.onrender.com/api/agendamentos/${appointmentToEdit.id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
...
export default function App() {
    ...
    const [user, setUser] = useState(JSON.parse(localStorage.getItem('clinicflow_user')));
    ...
    useEffect(() => {
        const fetchData = async (endpoint, setter) => {
            ...
            try {
                ...
                else console.error(`Falha ao buscar ${endpoint}: Status ${response.status}`);
            } catch (error) { console.error(`Falha ao buscar ${endpoint}:`, error); }
        };
    ...
    const renderPage = () => {
        if (user.perfil !== 'admin' && (currentPage === 'Dashboard' || currentPage === 'Serviços' || currentPage === 'Profissionais')) {
            setCurrentPage('Agenda');
            return <Agenda appointments={appointments} setAppointments={setAppointments} clients={clients} services={services} professionals={professionals} token={token} />;
        }

        switch (currentPage) {
            case 'Dashboard': return <Dashboard clients={clients} receivables={receivables} payables={payables} />;
            case 'Clientes': return <Clients clients={clients} setClients={setClients} token={token} />;
            case 'Serviços': return <Services services={services} setServices={setServices} token={token} />;
            case 'Profissionais': return <Professionals professionals={professionals} setProfessionals={setProfessionals} token={token} />;
            case 'Agenda': return <Agenda appointments={appointments} setAppointments={setAppointments} clients={clients} services={services} professionals={professionals} token={token} />;
